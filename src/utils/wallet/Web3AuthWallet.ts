...
const web3AuthInstances: { [key: string]: Web3Auth } = {};
...
export class Web3AuthWallet extends EVMWallet<Web3AuthConfig> {
  ...
  createConnectorFn() {
    ...
    if (!web3AuthInstances[id]) {
      web3AuthInstances[id] = new Web3Auth(this.connectorOptions);
    }
    ...
    return createConnector<IProvider, Properties>((config) => ({
      ...
      async disconnect() {
        ...
        await web3AuthInstances[id].logout();
      },
      async getAccounts() {
        const provider = await this.getProvider();
        if (!provider) throw new ProviderNotFoundError();
      ...
      async getProvider() {
        async function init() {
          if (!web3AuthInstances[id].connected) {
            await web3AuthInstances[id].init();
            await web3AuthInstances[id].connect();
          }

          return web3AuthInstances[id].provider!;
        }
      ...
      async switchChain({ chainId }) {
        ...
        await web3AuthInstances[id].switchChain({ chainId: toHex(chainId) });
        return chain;
      },
      async isAuthorized() {
        return web3AuthInstances[id].connected;
      },
